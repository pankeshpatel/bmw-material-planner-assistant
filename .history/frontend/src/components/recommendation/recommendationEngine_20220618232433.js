...
export const RecommendationEngine = (props) => {
  ...
  useEffect(async () => {
    ...
    for (let i = 0; i < data.result.length; i++) {
      //console.log("MATERIAL ENGINE: ", data.result[i]["material"]);
      materials.push(data.result[i]["material"]);
    }

    // for (let j = 0; j < materials.length; j++) {
    //   console.log("ARRAY MATERIALS: ", materials[j]);
    // }

    console.log("ARRAY MATERIALS: ", materials);


    setPlannerMaterials(materials);

    //console.log("MATERIAL ENGINE: ", data.result[0]["material"]);
